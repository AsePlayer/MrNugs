...
private:
	...
	vector<Weapon> cheapWeapons = { Weapon("Stick", 5, "\"Weapon Description #1\"", 50), Weapon("Branch", 15, "\"Weapon Description #2\"", 100), Weapon("Log", 30, "\"Weapon Description #3\"", 250) };
	vector<Weapon> expensiveWeapons = { Weapon("Expensive Stick", 44, "Expensive #1", 500), Weapon("Expensive Branch", 66, "Expensive #2", 1500), Weapon("Expensive Log", 99, "Expensive #3", 3000) };
	vector<Weapon> weaponsForSale;

	vector<Item> items = { Item("Ketchup Packet", 100, 50, "Heals 100 HP", "HP"), Item("Mustard Packet", 100, 50, "Recovers 100 MP", "MP"), Item("Ketchup Bottle", 250, 150, "Heals 250 HP", "HP"), Item("Mustard Bottle", 250, 150, "Recovers 250 MP", "MP") };
	...
	void shop(Player *player) {
		...
		while (!exit) {
			...
			if (option == -2) {
				...
				cout << "Something broke in the shop.";
			}
		}
	}
	...
	int displayDeals(Player *player) {
		...
		case 1:
			...
			if (option == 0) {
				...
				cout << "Would you like to sell your " << currentWeapon.getName() << " for " << currentWeapon.getPrice() / 2 << " gold and purchase " << weaponsForSale[option - 1].getName() << " for " << weaponsForSale[option - 1].getPrice() << " gold?" << endl;
				...
				if (confirm == 1) {
					...
					cout << endl << "Congratulations, you are now the proud owner of a " << weaponsForSale[option-1].getName() << endl;
